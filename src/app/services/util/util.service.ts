...
export class UtilService {
  ...
  // @return 2026.00001
  NuevoSemillero(id: string): string {
    var f = new Date();
    var anio = f.getFullYear().toString().substring(0, 4);
    return anio + "." + this.zfill(id, 5);
  }
  ...
  esFechaAntesMarzo2025(dateString: string): boolean {
    ...
    // Fecha de corte: 1 de Marzo de 2025 (Mes 2 en JS)
    // Si realmente querías Septiembre, cambia el 2 por un 8
    const cutoffDate = new Date(2025, 2, 1);

    // Comparar: Queremos saber si es ANTES, así que usamos "<"
