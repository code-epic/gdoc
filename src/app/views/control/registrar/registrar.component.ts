...
export class RegistrarComponent implements OnInit {
  ...
  public bzRegistrados: any[] = [];

  public bzNotaEntregas: any[] = [];
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (row instanceof HTMLElement) {
      row.style.setProperty(
        "--x",
        `${event.clientX - row.getBoundingClientRect().left}px`,
      );
    }
  }
  ...
  updateAllComplete() {
    this.allComplete =
      this.buzon != null && this.buzon.every((t) => t.completed);
  }
  ...
  someComplete(): boolean {
    ...
    return (
      this.buzon.filter((t) => t.completed).length > 0 && !this.allComplete
    );
  }
  ...
  async notaEntrega(id: string) {
    ...
    if (cantidad > 0) {
      ...
      this.lstNotaEntrega.forEach((e) => {
        ...
        this.xAPI.valores = "";
        this.xAPI.parametros =
          origen + "," + estatus + "," + this.llave + "," + usuario + "," + id;
