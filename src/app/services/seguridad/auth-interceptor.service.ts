  ...
  private procesarPeticion(
    ...
    return next.handle(request).pipe(
      catchError((err: HttpErrorResponse) => {
        if (
          (request.url && request.url.includes("federation/")) ||
          request.url.includes("federate/")
        ) {
          console.error("Error en petición federada (ignorado logout):", err);
