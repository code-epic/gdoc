...
export class ArchivoComponent implements OnInit, AfterViewInit {
  @ViewChild("modalSeleccionarArchivo") modalSeleccionarArchivo!: TemplateRef<any>;
  public modalRef: any;

  public estadoActual = 11;
  public nombreArchivoActual = "Archivo de Control de Gestión";

  public opcionesArchivo = [
    {
      id: 11,
      nombre: "Control de Gestión",
      subtitulo: "Seguimiento, proyectos y expedientes de gestión",
    ...
    },
    {
      ...
      subtitulo: "Correspondencias generales, actas e informes",
    ...
    },
    {
      id: 15,
      nombre: "Resoluciones",
      subtitulo: "Resoluciones aprobadas, normativas y expedientes",
      icono: "ni ni-badge",
      color: "#2dce89",
      lightBg: "#eafaf1"
    }
  ...
  public bzOriginal: any[] = []; //Listado Original
  public buzon: any[] = [];
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      this.openModalSeleccionArchivo();
    }, 150);
  }

  openModalSeleccionArchivo(): void {
    if (this.modalSeleccionarArchivo) {
      this.modalRef = this.modalService.open(this.modalSeleccionarArchivo, {
        centered: true,
        backdrop: "static",
        keyboard: false,
        size: "md",
        windowClass: "archivo-custom-modal"
      });
    }
  }

  seleccionarArchivo(opcion: any): void {
    this.estadoActual = opcion.id;
    this.nombreArchivoActual = opcion.nombre;
    if (this.modalRef) {
      this.modalRef.close();
    }
  ...
  updateAllComplete() {
    this.allComplete =
      this.bzRecibido != null && this.bzRecibido.every((t) => t.completed);
  }
  ...
  someComplete(): boolean {
    ...
    return (
      this.bzRecibido.filter((t) => t.completed).length > 0 && !this.allComplete
    );
  }
