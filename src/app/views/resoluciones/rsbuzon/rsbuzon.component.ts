...
export class RsbuzonComponent implements OnInit {
  ...
  public cmbAcciones = [
    ...
    { valor: "11", texto: "ARCHIVAR EN RESOLUCIONES", visible: "0" },
  ];

  public lstAcciones: any[] = [];
  public bzOriginal: any[] = [];
  public bzSubDocumentos: any[] = [];
  public bzRecibido: any[] = [];
  public lstRecibidos: any[] = [];
  public bzRecibidoResumen: any[] = [];
  public bzClasificar: any[] = [];
  public bzProcesados: any[] = [];
  public bzPendientes: any[] = [];
  public lst: any[] = [];
  public lstEstados: any[] = []; //Listar Estados
  public lstResponsable: any[] = [];
  public lstCedula: any[] = [];
  ...
  public currentFechaHoy: string = new Date().toLocaleDateString("es-VE");
  ...
  public lstAccionesMininisterial = [
    ...
    {
      valor: "13",
      texto: "RESOLUCIONES / JEFE DEL AREA SECRETARIA",
      visible: "1",
  ...
  public lstDigitalesDevuelto: any[] = [];
  clasificacion: any;
  vplazo: any;
  placement: any;
  public Grados: any;
  ...
  public getSubcasos(doc: any): any[] {
    if (!doc) return [];
    let list: any[] = [];
    if (doc.subdocumento) {
      try {
  ...
  groupAndPrepareFolders() {
    ...
    this.lstCarpetas.forEach((folder: any) => {
    ...
    this.groupedCarpetas = Object.keys(groups)
      .map((key) => {
        const componentCode = parseInt(key, 10);
        const componentName =
          this.componentMap[componentCode] || "Componente Desconocido";
        // Initialize display count for this group
        this.displayCounts[componentName] = this.itemsPerLoad;
        return {
          componentCode: componentCode,
          componentName: componentName,
          folders: groups[componentCode] as any[],
        };
      })
  ...
  async crearCarpeta() {
    ...
    this.xAPI.parametros = `'0',${elementos}cedula##${componente}cedula##${numero}cedula##${tipo}cedula##${estatus}cedula##${clasificacion}cedula##${prioridad}cedula##${observacion}cedula##${usuario}cedula##${idtrans}`;
  ...
  limpiarTexto(texto: any): string {
    ...
    try {
      ...
      // fallback if DOM element creation is unavailable
    }
  ...
  getDetalle(e): string {
    return e.tdoc == "PUNTO DE CUENTA" || e.tdoc == "MULTIPLE/PUNTO DE CUENTA"
      ? e.s_cuenta
      : e.numc;
  }
  ...
  getEstatusMinisterial(e): string {
    ...
    return this.lstAccionesMininisterial[pos] != undefined
      ? this.lstAccionesMininisterial[pos].texto
      : "";
  }
  ...
  getMotivo() {
    ...
    this.apiService.Ejecutar(this.xAPI).subscribe(
      (data) => {
        ...
        this.blReserva = true;
        this.ngxService.stopLoader("loader-buscar");
        if (this.IResolucion.solicitud.toString() == "7")
          this.blReservaAux = true;
      },
      (err) => {
        console.error(err);
      },
