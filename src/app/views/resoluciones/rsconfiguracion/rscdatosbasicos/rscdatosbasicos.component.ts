...
export class RscdatosbasicosComponent implements OnInit {
  ...
  AceptarCambios() {
    ...
    this.nacimiento = NgbDate.from(
      this.formatter.parse(this.ipsfa_fechanacimiento_unix)
    );
    this.ingreso = NgbDate.from(
      this.formatter.parse(this.ipsfa_fechaingreso_unix)
    );
    this.ascenso = NgbDate.from(
      this.formatter.parse(this.ipsfa_fechaascenso_unix)
    );
  }
