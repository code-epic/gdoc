...
export class DashboardComponent implements OnInit {
  ...
  isStatsAllowed(): boolean {
    const perfil = (sessionStorage.getItem("perfil") || "").toUpperCase();
    const cargo = (this.loginService.Usuario?.cargo || "").toUpperCase();

    if (
      cargo === "ADMINISTRADOR" ||
      this.loginService.Usuario?.tipo?.toUpperCase() === "ADMINISTRADOR"
    )
      return true;
    ...
    return allowed.some((val) => perfil.includes(val));
  }
}
