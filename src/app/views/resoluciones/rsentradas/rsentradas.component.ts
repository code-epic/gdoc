...
export class RsentradasComponent implements OnInit {
  ...
  lstR: any[] = []; //Objeto Remitente
  lstU: any[] = []; //Objeto Unidad
  lstC: any[] = []; //Objeto Comando
  lstCuenta = []; //Objeto Unidad

  lstHzAdjunto: any[] = []; //Historico de documentos adjuntos
  lstTraza: any[] = [];
  lstHistorial: any[] = [];
  lstImg: any[] = [];
  titulo = "Documento";
  ...
  Configuracion: any;
  remitenteCtrl = new FormControl();
  unidadCtrl = new FormControl();
  comandoCtrl = new FormControl();
  filteredRemitentes: Observable<any[]>;
  filteredUnidades: Observable<any[]>;
  filteredComandos: Observable<any[]>;
  ...
  ngOnInit(): void {
    ...
    this.filteredRemitentes = this.remitenteCtrl.valueChanges.pipe(
      startWith(""),
      map((value) => (typeof value === "string" ? value : value?.nomb || "")),
      map((name) =>
        name ? this._filterConfiguracion(name, this.lstR) : this.lstR.slice(),
      ),
    );

    this.filteredUnidades = this.unidadCtrl.valueChanges.pipe(
      startWith(""),
      map((value) => (typeof value === "string" ? value : value?.nomb || "")),
      map((name) =>
        name ? this._filterConfiguracion(name, this.lstU) : this.lstU.slice(),
      ),
    );

    this.filteredComandos = this.comandoCtrl.valueChanges.pipe(
      startWith(""),
      map((value) => (typeof value === "string" ? value : value?.nomb || "")),
      map((name) =>
        name ? this._filterConfiguracion(name, this.lstC) : this.lstC.slice(),
      ),
    );
  ...
  dwUrl(ncontrol: any, archivo: string): string {
  ...
  async SubirArchivo() {
    if (!this.Entradas.cuenta || this.Entradas.cuenta.trim() === "") {
      ...
      this.toastrService.error(
        "GDOC MPPD El campo cuenta es obligatorio",
        "Error",
      );
      return;
    }
    ...
    try {
      await this.apiService.EnviarArchivos(frm).subscribe((data) => {
        this.Entradas.archivo =
          this.archivos[0] == undefined ? "" : this.archivos[0].name;
        if (!this.editar) {
          this.guardar();
        } else {
          this.actualizar();
      ...
      this.toastrService.error(
        error,
        `GDoc MPPD Insertar resuelto subir el archivo`,
      );
    }
  }
  ...
  guardar() {
    ...
    this.Entradas.ecomponente = parseInt(
      this.Resolucion.codigo_componente.toString(),
    );
  ...
  private _filterConfiguracion(name: string, list: any[]): any[] {
    const filterValue = name.toLowerCase();
