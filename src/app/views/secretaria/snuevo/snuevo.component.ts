...
export class SnuevoComponent implements OnInit {
  ...
  public estadoOrigen = 3;
  public estatusOrigen = 3;
  ...
  async ngOnInit() {
    ...
    this.fplazoDate = NgbDate.from(
      this.formatter.parse(this.utilService.FechaActual(5)),
    );
  ...
  registrar() {
    if (this.lstCuenta.length == 0) {
      this.toastrService.info(
        "Debe ingresar detalles de la cuenta",
        `GDoc Wkf.Documentos`,
      );
  ...
  async actualizarDocumentos() {
    ...
    if (this.WAlerta.documento != 0)
      this.WAlerta.fecha =
        typeof this.fplazoDate === "object"
          ? this.utilService.ConvertirFecha(this.fplazoDate)
          : this.fplazoDate;
