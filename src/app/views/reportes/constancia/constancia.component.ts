...
import {
  IDocumento,
  IWKFAlerta,
} from "../../../services/control/documentos.service";
...
export class ConstanciaComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.rutaActiva.snapshot.params.id != undefined) {
      ...
      this.lstUsuario =
        sessionStorage.getItem("CEP_CUsuario") != undefined
          ? JSON.parse(atob(sessionStorage.getItem("CEP_CUsuario")))
          : [];
        ...
        data.Cuerpo.forEach(async (e) => {
          ...
          const codigo =
            this.Doc.nexpediente != ""
              ? this.Doc.codigo + " / " + this.Doc.nexpediente
              : "";
          ...
          this.Doc.contenido = this.domSanitizer.sanitize(
          ...
          const traza =
            this.Doc.traza != null ? JSON.parse(this.Doc.traza) : [];
          // console.log(traza);

          if (traza.length == 0) {
            this.nexpediente =
              this.Doc.estado_doc != ""
                ? this.Doc.estado_doc + " / " + this.Doc.estatus_doc
                : "";
          } else {
            this.btraza = true;
  ...
  getDepartamento(e): string {
    return e.toUpperCase() == "REGISTRAR"
      ? "REGISTRO Y ESCANEO"
      : e.toUpperCase();
  }
